 ...
 * @param array $series pdo object
 * @param array $names_user with names from users from the db
 * @return string
 */
function get_serie_table($series, $names_user)
{
    $i = 0;
    $table_exp = '
...
//        foreach ($names_user as $user => $names) {
        $table_exp .= '
        <tr>
            <th scope="row">' . $value['name'] . '</th>
            <td>' . $names_user[$i]['firstname'] . ' ' . $names_user[$i]['lastname'] . '</td>
 ...
 * @param integer $user_id user that adds the series
 ...
 * @param object $pdo db object
 * @param array $serie_info post array
 * @param integer $user_id user id from the session info
 * @return array
 */
function update_serie($pdo, $serie_info, $user_id)
{
    ...
    if ($serie_info['user'] =! $user_id){
        return [
            'type' => 'danger',
            'message' => 'There was an error. You are not allowed to edit this series.'];
    }
...
/**
 * Count the number of users that are registered on Series Overview
 * @param object $pdo database object
 * @return mixed
 */
...
/**
 * Generates an array with the first- and last name
 * that belong to a given user_id
 * @param object $pdo db object
 * @param int $user_id id from the serie
 * @return mixed
 */
function get_user_names($pdo, $user_id)
{
...
/**
 * Generates an array with the username
 * that belong to a given user_id
 * @param object $pdo db object
 * @param int $user_id
 * @return mixed
...
function get_username($pdo, $user_id)
{
    ...
    foreach ($username as $key => $value) {
        $username_exp[$key] = htmlspecialchars($value);
    }
    return $username_exp;
}
 ...
 * Log a user in
 * @param object $pdo db object
 * @param mixed $form_data POST data with user info
 * @return mixed
 */
...
/**
 * Logs the user out
 * @return array message if logged out successfully
 */
function logout_user()
{
    try {
        ...
        return [
            'type' => 'danger',
            'message' => sprintf('Logging out went wrong. Error: ', $e->getMessage())
        ];
    }
}
